...
with open('utterance/compare-srilm-slm.u.slm_fulluniform.probs', 'r', encoding='iso-8859-1') as f:
    for line in f:
        if "<s> <s> <s>" in line:
            utterances.append(cur_utterance)
            cur_utterance = []
        cur_utterance.append(line.strip().split(" = "))
...
with open('concat/compare-srilm-slm.c.slm_fulluniform.probs', 'r', encoding='iso-8859-1') as f:
    for line in f:
        all_concat.append(line.strip().split(" = "))

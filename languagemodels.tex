...
In \cref{fig:maxgrowth} we added the maximum number of possible $n$-grams to the number of observed $n$-grams. The graph shows that this feature space can be really sparse, and that the observed number of $n$-grams, and more notably the $5$-grams, are even below the maximum number of unigrams. Hence the chance of running into an observed $n$-gram is very likely, and that is why further in this chapter we show some countermeasures to improve MLE to deal with this issue.
